...
BEGIN
  -- Check ownership
  IF NOT EXISTS (SELECT 1 FROM public.routines WHERE id = p_routine_id AND user_id = p_user_id) THEN
  ...
  -- 2. Lock the routine row for update to prevent races
