...
export async function updateTask(req, res) {
  ...
  try {
    ...
    if (status === 'completed' && data.recurrence_pattern) {
      try {
        const updatedTaskFromRecurrence = await processRecurringTask(data, token);
      } catch (recError) {
        logger.error('Error processing recurring task:', recError);
      }
    }
...
export async function getNextFocusTask(req, res) {
  ...
  try {
    ...
    if (travel_preference === 'home_only') {
      query = query.or('location.is.null');
    }
